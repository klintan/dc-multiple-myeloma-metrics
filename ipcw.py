...
def ipcw(formula, data, method, args, times, subjectTimes, what, subjectTimesLag = 1,):
    ...
    if not what:
        raise(all(match(what, c("IPCW.times", "IPCW.subject.times"))))
    ...
    ~1, data = wdata, reverse = TRUE, caseweights = FW[i,]), times = times)
    })
    }
    else
    IPCW.times < - NULL
    ...
    {
        ## browser()
        prodlim:: predictSurvIndividual(prodlim::prodlim(Hist(time, status)
    ~1, data = wdata, reverse = TRUE, caseweights = FW[i,]), lag = 1)[i]
    })
    }
    else
    IPCW.subject.times < - NULL
    ...
    # weigths at subject specific event times
    if (match("IPCW.subject.times", what, nomatch=FALSE)){
    IPCW.subject.times < - prodlim::
        predictSurvIndividual(fit, lag=lag)
    }
    else
    IPCW.subject.times < - NULL
    out < - list(IPCW.times = IPCW.times,
    ...
    if (match("IPCW.times", what, nomatch=FALSE)){
    IPCW.times < - rms::
        survest(fit, newdata=wdata, times=times, se.fit = FALSE)$surv
    }
    else
    IPCW.times < - NULL
    #  weigths at subject specific event times
    if (match("IPCW.subject.times", what, nomatch=FALSE)){
    if (lag == 1)
    IPCW.subject.times < - rms::
        survest(fit, times=subject.times - min(diff(c(0, unique(subject.times)))) / 2, what='parallel')
    else if (lag == 0){
    IPCW.subject.times < - rms::
        survest(fit, times=subject.times, what='parallel')
    }
